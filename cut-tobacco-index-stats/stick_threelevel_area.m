...
rootdir = 'G:\workspace\数据存储\上烟制丝-卷包项目\在线实验-烟丝长度检测';
...
%                 imshow(skL)
                ...
                LenAreas(:,t) = [LenVal;AreaVal];
            end
            
            % 保存结果
            imgName = strcat(subsubdir(j).name,subsubdirfiles(s).name(end-6:end)); % 每张图像名称，去掉文件后缀名
                    
            desPath = strcat(curpath,'\results\',subdir(i).name,'.xlsx');
            writematrix(imgName,desPath,'Sheet',j-2,'Range',strcat('A',num2str(2*s-1)));   % 写入图像名称
            writematrix(LenAreas,desPath,'Sheet',j-2,'Range',strcat('B',num2str(2*s-1)));% 写入烟丝长度、面积
        end
    end
end